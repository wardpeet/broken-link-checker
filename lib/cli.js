...
import {BROKEN_REASON, EXCLUDED_REASON, HTML_INDEX, HTML_TAG_NAME, HTTP_RESPONSE_WAS_CACHED, IS_BROKEN, ORIGINAL_URL, REBASED_URL, WAS_EXCLUDED} from "./internal/Link";
...
const argsToOptions = args =>
{
  ...
  return Object.entries(args).reduce((opts, [argName, argValue]) => {
    if (argName === "follow") {
      opts.honorRobotExclusions = false;
    }
...
const run = () =>
{
	...
	.on(JUNK_EVENT, link =>
	{
		if (link.get(HTML_TAG_NAME) === "button") {
			link.set(IS_BROKEN, false);
			link.set(WAS_EXCLUDED, true);
		}
